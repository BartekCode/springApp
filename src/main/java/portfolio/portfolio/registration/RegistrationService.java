...
public class RegistrationService {
    ...
    public String req(RegistrationRequest request){
        User userToSave = userMap(request);
        String token = userService.addUser(userToSave);
    ...
    public User userMap (RegistrationRequest registrationRequest){//mapowanie bardzo wazne
        return new User(registrationRequest.getFirstName(),registrationRequest.getLastName(),
                registrationRequest.getEmail(),registrationRequest.getPassword(),Role.USER);
    }

    public String sendNewToken(RegistrationRequest request){
        User user = userMap(request);
        confirmationTokenService.deleteOldToken(user);
        String token = userService.reSendToken(user);
        String link = "http://localhost:8080/registration/confirm?token=" + token;
        emailSender.send(
                request.getEmail(),
                buildEmail(request.getFirstName(), link));
    ...
    public String confirmToken(String token){
        ...
        userService.enableUserr(confirmationToken.getUser().getEmail());
